...
def main () :
    ...
    assignCodes(tree)
    print "\nCoder assigned for letters are\n", codes

    small = encode(str)
    original = decode (tree, small)
    print "\nOriginal text length", len(str)
    print "\nRequires %d bits. (%d bytes)" % (len(small), (len(small)+7)/8)
    print "\nThe decoded version is:", original
    print "\nThe encoded version is:", small
    print "\nRestored matches original is", str == original
